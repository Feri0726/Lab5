/* Task description:
Write a program that creates an array of ten elements! Assign initial values to the array elements, too! 
(So you do not need to read them one by one.) Print the array to the screen!
...
when it is the last; when it is somewhere in between them!) Print the array in such manner that a special mark is printed at the least element.
